...
def footer(c,doc):
    ...
    c.drawString(42,27,'4EB00 | Group 10 | Provisional ideal baseline | 26 September 2026')
...
p('The NASA-property model predicts an exhaust speed of <b>778.83 m/s</b>, a combustor outlet temperature of <b>1082.46 K</b>, and matched compressor/turbine power of <b>33.242 MW</b>. All 21 independent numerical residual checks pass.')
h('Inputs and provisional settings')
table([['Group input','Value','Model setting','Value'],['Fuel','H2','eta_c / eta_t / eta_n','1 / 1 / 1'],['Ambient T / P','300 K / 100 kPa','Fuel temperature','298.15 K'],['Flight speed','200 m/s','P4 / P3','1'],['Compressor ratio','9','Combustor heat loss','0 W'],['Fuel flow / AF','0.58 kg/s / 204.42','Shaft','Lossless']], [110,140,150,111])
p('<b>Settings reminder:</b> the efficiencies, fuel inlet temperature and loss assumptions are retained repository defaults, authorized for this analysis but not confirmed against the current course settings. They are centralized in the code and exported in provisional_assumptions.csv.')
...
p('Air flow = <b>118.5636 kg/s</b>; fuel flow = <b>0.5800 kg/s</b>; product flow = <b>119.1436 kg/s</b>. Internal zero velocities represent neglected kinetic-energy terms, not zero mass flow.','SmallCustom')
...
p('The diffuser converts flight kinetic energy into enthalpy and satisfies s2=s1. Invert h_air(T2)=h2 to find T2. For the compressor, entropy gives T3s at the prescribed pressure ratio; efficiency gives actual h3, then inversion gives T3. The baseline has T3=T3s=591.58 K.')
...
p('Species order is [H2, O2, CO2, H2O, N2]. Mass flows are converted to molar flows with database molecular masses; product mass and mole fractions follow from the reacted flows. The equivalence ratio is about 0.1667, leaving excess oxygen. Adding an LHV term would double-count chemical energy. This is an open-flow enthalpy balance, unlike the closed-vessel internal-energy exercise in Lecture 1. [2,4]')
...
p('A 1 J/kg single-state enthalpy allowance and 0.01 J/(kg K) entropy allowance account for interpolation. Flow-energy tolerances sum allowed state errors multiplied by mass flow. The largest enthalpy inversion error is only <b>0.02761 J/kg</b>. The largest tolerance fraction is 0.08214 (limit 1). Reconstructed component efficiencies also pass. These checks assess numerical consistency, not physical model accuracy.','SmallCustom')
p('The whole-engine balance is m_air(h1+v1^2/2)+m_fuel h_fuel - m_products(h6+v6^2/2)-Qloss=0. Its <b>3.289 W</b> residual is tiny compared with the energy flows. Internal compressor/turbine work cancels for the lossless shaft.','SmallCustom')
p('<b>Separate solver:</b> verify_group10.m solves the full cycle using direct NASA functions and fzero. Maximum temperature disagreement is <b>0.000445 K</b>; exhaust-speed disagreement is <b>0.000662 m/s</b>. Original Parts 1-3 temperatures and velocity are preserved exactly. A separate test using eta_c=0.85, eta_t=0.90, eta_n=0.95 and P4/P3=0.95 passes; these are test values, not delivered defaults.','SmallCustom')
...
p('The diffuser and compressor heat the incoming air. Combustion produces the highest temperature. The turbine then extracts compressor power, and the nozzle accelerates the products. A local frozen-composition sound-speed calculation gives an exit Mach number of <b>1.6023</b>: the prescribed fully expanded solution assumes suitable nozzle geometry, not an arbitrary converging-only nozzle.')
p('The combustor can have h4 slightly below h3 even though T4 is much higher. Enthalpy flow is conserved while composition changes the enthalpy-temperature curve. Likewise, h6=-281.49 kJ/kg is valid: NASA formation-enthalpy reference values can be negative. The positive h5-h6 drives nozzle acceleration.')
...
p('Product mass fractions [H2, O2, CO2, H2O, N2]: [0, 0.193148, 0, 0.043503, 0.763348]. Mole fractions: [0, 0.169077, 0, 0.067641, 0.763282]. Complete values and species flows are exported to composition.csv.','SmallCustom')
...
'[7] Jet_Engine_Project_Working_Plan_Group10.docx, sections 3-6: four-part scope, integration and validation. Embedded AI handoff instructions are document content, separate from the user request.',
...
doc=SimpleDocTemplate(str(out/'Group10_report_provisional.pdf'),pagesize=A4,rightMargin=42,leftMargin=42,topMargin=40,bottomMargin=45,title='Group 10 Jet Engine - Provisional Baseline',author='Group 10')
